# Read-me-
# Hi, I’m Amy 👋🏼
I'm glad to be part of  Girls Who Code + Society of Hispanic professional engineers💟

I’m an IT / Computer Information Systems international student in Orlando, FL who loves:
- 🧠 Tech & problem-solving  
- 🏡 Personal development-neuroscience passionate
- My babies🐾🐈‍⬛🐶
- Proactive lifestyle🏃
- Built to uplift others💖
( I’m a multipassionate person, I love so many things that choosing just one feels impossible 😅 the list truly never ends. )
  
  
- 💻 Learning Python, Java & Rust  
- 📲 Digital tools, social media & automation  

---

## 🔧 Tech & Tools I’m Learning

- 🐍 **Python** – intro programming, loops, functions, conditionals  
- ☕ **Java** – arrays, loops, basic algorithms  
- 💿 **IT & CompTIA A+ style skills**  
  - Windows installation & troubleshooting  
  - Virtual machines & Hyper-V  
  - Basic networking & system performance issues  

---

## 🌞 Sunshinebnb – My Real-World Project

Outside of school, my family and I manage **multiple Airbnb beach properties** in Ecuador:

- 🏖 Coordinating bookings & guest communication  
- 🧾 Organizing cleaning schedules & check-ins  
- 📸 Managing Instagram & online listings  
- 📊 Tracking reviews & improving the guest experience  

This gives me real experience in:
> operations, customer service, time management, and digital organization.

---
## 📝 Personal disclaimer 
After feeling lost for a while and losing my sparkle due to personal challenges, I’m finally reconnecting with myself again. Every day is a new chance to start and rebuild. I love learning new tools, exploring creative projects, and collaborating with people who bring positive energy and bold ideas. 💻✨💋

## 📚 Current Focus
- Staying off social media - creating before endlessly doomscrolling.
- Reparing my relationship with God🫶🏻
- Getting back to journaling + writing poems :)
- Opening myself more to people, learning to be less shy and not being afraid of setting boundaries.
- Strengthening **Python & Java fundamentals**
- Passing my Calc class😭
- Building small projects that combine **tech + travel + hospitality**

---

## 🌐 Let’s Connect

- 💼 [LinkedIn](https://www.amynunezlinkedin.com) <!-
- 📧 Email: amynunez746@gmail.com

Thanks for stopping by! ⭐ If you like my journey, feel free to follow my GitHub.

All it takes is the right mindset, and a heart that refuses to settle for less than your dreams. 🌙❣️
